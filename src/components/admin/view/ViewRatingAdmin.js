...
export default function ViewRatingAdmin(){
    const [rating, setRating] = useState([]);
    const [load, setLoad]=useState(true)
    let {id}=useParams()
      useEffect(() => {
        const fetchBookings = () => {
                var ratingsCollection = query(collection(db, 'ratings'), where("bookingId","==",id), orderBy("createdAt", "desc"));
        ...
        <>
          ...
          <div className="row">
            <div className="col-md-12">
                <h1 className="client_taital">Customers Rating</h1>
            </div>
        </div>
